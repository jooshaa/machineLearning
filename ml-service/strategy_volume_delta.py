        ...
        if touched:
            ...
            if not post_signal.empty:
                ...
                if t_tp < t_sl and t_tp != sentinel:
                    ...
                    last_price = float(post_signal['price'].values[-1])
                    last_time = post_signal.index.values[-1]
                    profit_points = (last_price - entry_price) if imp_type == 'up' else (entry_price - last_price)
                    r_multiple = round(float(profit_points / sl_distance), 2)
                    ...
                    if r_multiple > 0:
                        ...
                        outcome = 'timeout'
                        result = '0R'
                    bars_to_outcome = int((last_time - entry_time) / np.timedelta64(1, 'm'))
