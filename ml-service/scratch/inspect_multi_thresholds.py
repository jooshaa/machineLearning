...
for d in dates:
    ...
    try:
        ...
        print(f"Processing stream {d}...")
        events = process_mbo_stream(mbo_df)
        events_df = pd.DataFrame(events)
        
        print(f"Extracting features {d}...")
        ...
        if not val:
            print(f"Error: VAL not calculated for {d}.")
            continue

        near_val = events_df[abs(events_df['price'] - val) <= 50].copy()
        if near_val.empty:
            print(f"No events near VAL ({val}) for {d}.")
            continue
        ...
        print(f"=== Фактические значения у VAL ({val:.2f}) ===")
