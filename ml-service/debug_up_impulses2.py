...
for imp in up_imps:
    ...
    zone_low = price_stop - (abs(imp['price_stop'] - imp['price_start']) * 0.3)
    stopping_profile = profile[(profile.index >= zone_low) & (profile.index <= price_stop + 10)]
    print(f"\nUP impulse stopping profile (top 3 prices by volume):")
